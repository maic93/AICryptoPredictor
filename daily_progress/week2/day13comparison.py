...
def plot(results, y_test, all_preds, dates):
    ...
    axes[0].set_title("Model Leaderboard — MAE (lower = better) 🏆", fontweight="bold")
    ...
    for (name, preds), color in zip(all_preds.items(), line_colors):
        axes[1].plot(dates, preds, linewidth=1, linestyle="--", color=color, label=name, alpha=0.8)
...
def run():
    ...
    plot(results, y_test, all_preds, dates_test)
