...
def plot(y_test, preds_dict, ensemble_preds, dates, weights):
    ...
    for (name, preds), color in zip(preds_dict.items(), colors):
        axes[0].plot(dates, preds, linewidth=1, linestyle=":", color=color, alpha=0.6, label=name)
    axes[0].plot(dates, ensemble_preds, color="#E74C3C", linewidth=2, linestyle="--", label="Ensemble 👑")
    axes[0].set_title("Ensemble vs Individual Models", fontweight="bold")
    axes[0].legend(fontsize=8); axes[0].grid(alpha=0.3)
    names = list(preds_dict.keys()) + ["Ensemble"]
    maes  = [mean_absolute_error(y_test, p) for p in preds_dict.values()] + [mean_absolute_error(y_test, ensemble_preds)]
...
def run():
    ...
    print(f"\n  {'Ensemble':>15} MAE=${ens_mae:,.2f} 👑")
    plot(y_test, preds_dict, ensemble_preds, dates_test, weights)
    print("\n✅ Ensemble complete!")
