...
Called by the rerun_day.yml GitHub Actions workflow.

Phase 1: days 1-56  (curriculum scripts)
Phase 2: day 57     (re-run today's live market report)

Usage: python .github/scripts/rerun_day.py <day_number>
       python .github/scripts/rerun_day.py 57   # re-run market report
"""
import sys
import os
import subprocess
...
CURRICULUM = {
    ...
    # ── PHASE 2 ──────────────────────────────────────────────────────────────
    57: ("Phase 2 — Re-run today's live market report",                "daily_progress/phase2/daily_market_report.py"),
}
...
def main():
    ...
    if day not in CURRICULUM:
        print(f"ERROR: Day {day} not in range (1-57)")
        sys.exit(1)

    task, script = CURRICULUM[day]
    print(f"Re-running Day {day}: {task}")
    print("=" * 55)
    ...
    # For phase2, report goes to market_reports/ with today's date
    if day == 57:
        os.makedirs("market_reports", exist_ok=True)
        path = f"market_reports/{date.today()}.md"
    else:
        os.makedirs("reports", exist_ok=True)
        path = f"reports/day{day:02d}.md"

    report = (
        f"# {'Market Report' if day == 57 else f'Day {day:02d}'} Report\n\n"
        f"**Task:** {task}\n"
        f"**Script:** `{script}`\n"
        f"**Date:** {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')} *(re-run)*\n\n"
        f"## Output\n\n```\n{output.strip()}\n```\n"
    )
