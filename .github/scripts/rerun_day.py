...
CURRICULUM = {
    ...
    13: ("Day 13 — Model comparison leaderboard",                      "daily_progress/week2/day13comparison.py"),
...
def main():
    day = int(sys.argv[1]) if len(sys.argv) > 1 else 16
    if day not in CURRICULUM:
        print(f"ERROR: Day {day} not in curriculum (1-28)")
        sys.exit(1)
...
**Task:** {task}
**Script:** `{script}`
**Date:** {datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")} *(re-run with bug fix)*
