...
> A self-running AI cryptocurrency system that evolves in three phases — from an 8-week ML curriculum to live daily market intelligence — with every step auto-committed via GitHub Actions.
...
|-------|----------|--------------|
| **Phase 1** | 56 days (8 weeks) | Build the full AI crypto system — data, ML, deep learning, production |
| **Phase 2** | 30 days | Live daily market reports — real prices, signals, markdown only |
| **Phase 3** | 90 days | Enhanced reports — Fear & Greed, daily model retraining, weekly deep dives |

---

## Phase 1 — 8-Week ML Curriculum
...
<details>
<summary>Phase 1 Daily Checklist (56 days) — click to expand</summary>
...
## Phase 2 — 30-Day Live Market Reports

Every day a new `market_reports/YYYY-MM-DD.md` was committed with:

- Live prices + RSI, MACD, Bollinger Bands for BTC, ETH, SOL, BNB, ADA, DOT
- Buy/Sell/Hold signals with confidence scores
- OHLC table and market narrative
- Running signal log and performance history in `data/`

No images — pure markdown, zero repo bloat.

---

## Phase 3 — 90-Day Enhanced Daily Reports *(active)*

Every morning at 7AM UTC GitHub Actions runs automatically and commits a richer report:

### Daily (every day)
- **Live prices & signals** — RSI, MACD, Bollinger Bands, Buy/Sell/Hold for 6 coins
- **Real Fear & Greed Index** — fetched live from [alternative.me](https://alternative.me/crypto/fear-and-greed-index/) API
- **XGBoost retrained daily** — trained on latest 2 years of BTC data, MAE and R² logged
- **Model drift detection** — alerts when accuracy degrades vs 7-day average
- **BTC tomorrow prediction** — fresh AI price prediction every morning
- **OHLC table** — open, high, low, close for all 6 coins

### Every Sunday
- **Weekly deep dive** — full week summary added to that day's report
- Signal accuracy review — were last week's signals correct?
- Hypothetical portfolio simulation — how would each strategy have performed?
- Model performance trend — is the AI getting better or worse?

### Data logged automatically
```
data/
├── signal_log.json         ← every day's signals for all coins
├── performance_log.json    ← daily prices, RSI, return averages
├── model_performance.json  ← daily MAE/R² tracking (model drift)
└── weekly_reviews.json     ← Sunday deep dive summaries
```
...
├── daily_progress/
│   ├── week1/ — week8/      # 56 Phase 1 scripts
│   ├── phase2/              # daily_market_report.py
│   └── phase3/              # phase3_daily_report.py
├── market_reports/          # Daily .md reports (Phase 2 & 3, auto-generated)
├── reports/                 # Phase 1 daily reports + charts
├── data/                    # JSON logs + CSV datasets
